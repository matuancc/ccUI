3. 集成热更新
安装与注册 CodePush
在终端输入npm install -g code-push-cli
安装完毕后，输入code-push -v查看版本，如看到版本号则代表安装成功
在终端输入 code-push register，会在浏览器自动打开注册页面让你选择授权账号，授权通过之后 CodePush 会告诉你 access key，复制此可以到终端即可完成注册
然后输入code-push login进行登录，登录成功会提示 successfully logged-in
为了让 CodePush 服务器知道你的 app，我们需要向它注册 app：在终端输入code-push app add <appName> <os> react-native，如: code-push app add animalHusbandryCollectorApp android react-native。一般appName就是项目中app.json文件中的name。注册完成之后会返回一套 deploymentKey，请记录下该 key，会在后面步骤中用到。如果 android 和 iOS 都需要热更新功能，需要分别生成一套 key
...
Info.plist 执行命令：appcenter codepush deployment list -a <ownerName>/<appName> -k查看 server 上的 deployment key 在Info.plist文件里面新建一个键值对：
<key>CodePushDeploymentKey</key>
<string>Your Deployment Key</string>
打开 xcode，选中自己的项目，选中project下的项目名称，选择Info标签页，点击Configurations下的加号，选择Duplicate "Release" Configuration 命名为Staging.
数组插入
选择Build Settings标签页，点击加号，选择Add User Defined Setting用.xcodeproj 打开项目，先把 Debug/Release/Staging 的值配置好： Debug: $(BUILD_DIR)/$(CONFIGURATION)$(EFFECTIVE_PLATFORM_NAME) Release: $(BUILD_DIR)/$(CONFIGURATION)$(EFFECTIVE_PLATFORM_NAME) Staging: $(BUILD_DIR)/Release$(EFFECTIVE_PLATFORM_NAME) 然后把配置名称改成：CONFIGURATION_BUILD_DIR，然后再切换回.xcworkspace 打开项目
数组插入
再次选择Add User Defined Setting，输入CODEPUSH_KEY，通过命令appcenter codepush deployment list -a <ownerName>/<appName> -k把Staging和Production的 deployment key 分别填到CODEPUSH_KEY下匹配的属性里。

打开Info.plist，把之前的CodePushDeploymentKey的值改成$(CODEPUSH_KEY)
...
代码中使用
在项目中增加文件（可以直接拷贝）：scripts/codePush/index.js
'use strict';
...
main();
在项目中增加文件：scripts/codePush/makePush.js
...
由于热更新时我们只能针对特定的版本进行热更新，所以为了保证后续用户手机上 app 版本的一致性，我们推荐使用强更新和热更新相结合的方式。当有版本更新时，我们推荐使用强更新功能，当需要对某个版本进行 bug 修复时，我们推荐使用热更新功能。
...
发布热更新：在项目下执行命令：yarn codePush 然后按照步骤执行即可
